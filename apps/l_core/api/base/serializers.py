...
#         if not self.detail  and hasattr(self,'list_serializer_class') and self.action != 'metadata':
...
class CoreOrganizationSerializer(DynamicFieldsModelSerializer):
    class Meta:
        ...
        fields = (
            'name', 'full_name', 'address', 'edrpou', 'phone', 'fax', 'email', 'site', 'work_reason', 'region',
            'district',
            'group_organization', '__str__', 'id', 'status', 'register_date', 'sert_name',
            'main_unit', 'main_unit_state', 'main_activity_text', 'settlement_account',
            'note', 'mfo', 'certificate_number', 'bank_name', 'bank', 'ipn', 'statute_copy')
...
class GetUserPermissions(APIView):
    ...
    def get(self, request):
        ...
        user_data = CoreUser.objects.select_related('organization').get(id=user.id)
