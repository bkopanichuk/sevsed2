...
class CreateInitialTemplates():
    def __init__(self, organization):
        self.i = FakeInstance(organization)
        self.organization = organization
    ...
    def create_base_foulder(self):
        base_dir = os.path.join(MEDIA_ROOT, get_upload_template_path(self.i, ''))
        Path(base_dir).mkdir(parents=True, exist_ok=True)

    def create_initial_act(self):
        act_default_template = os.path.join(MEDIA_ROOT, 'default_doc_templates', 'act.docx')
        related_template_path = get_upload_template_path( self.i, 'act.docx')
        act_template_path = os.path.join(MEDIA_ROOT, related_template_path)
        shutil.copy2(act_default_template, act_template_path)
        if not TemplateDocument.objects.filter(organization=self.organization,
                                        related_model_name='registeract').exists():
    ...
    def create_initial_invoice(self):
        ...
        related_template_path = get_upload_template_path( self.i, 'invoice.docx')
        act_template_path = os.path.join(MEDIA_ROOT, related_template_path)
        shutil.copy2(act_default_template, act_template_path)
        if not TemplateDocument.objects.filter(organization=self.organization,
                                               related_model_name='registeract').exists():
